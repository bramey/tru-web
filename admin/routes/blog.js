...
exports.preview = function(req,res){
  Blog.findById(req.query.id)
    .then(function(blog){
      if(!blog) throw new Error('Blog entry not found')
      res.render('blog/preview',{blog: blog})
    })
    .catch(function(err){
      res.render('error',{error: err})
    })
...
exports.save = function(req,res){
  ...
  Blog.findById(data.id)
    .then(function(blog){
      ...
      if(data.author) blog.author = data.author
      if(data.authorUrl) blog.authorUrl = data.authorUrl
      if(data.coverPhotoUrl) blog.coverPhotoUrl = data.coverPhotoUrl
